...
void BloomEffect::Init(unsigned width, unsigned height)
{
	...
	_shaders[index2]->LoadShaderPartFromFile("shaders/BloomHighPass.frag.glsl", GL_FRAGMENT_SHADER);
	index2++;
	_shaders.push_back(Shader::Create());
	_shaders[index2]->LoadShaderPartFromFile("shaders/passthrough_vert.glsl", GL_VERTEX_SHADER);
	_shaders[index2]->LoadShaderPartFromFile("shaders/BlurHorizontal.frag.glsl", GL_FRAGMENT_SHADER);
	index2++;
	_shaders.push_back(Shader::Create());
	_shaders[index2]->LoadShaderPartFromFile("shaders/passthrough_vert.glsl", GL_VERTEX_SHADER);
	_shaders[index2]->LoadShaderPartFromFile("shaders/BlurVertical.frag.glsl", GL_FRAGMENT_SHADER);
	index2++;
	_shaders.push_back(Shader::Create());
	_shaders[index2]->LoadShaderPartFromFile("shaders/passthrough_vert.glsl", GL_VERTEX_SHADER);
	_shaders[index2]->LoadShaderPartFromFile("shaders/BloomComposite.frag.glsl", GL_FRAGMENT_SHADER);
